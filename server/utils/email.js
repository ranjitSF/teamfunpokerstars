  ...
  try {
    ...
    const emailPromises = players.map((player) => {
      const mailOptions = {
        ...
        to: player.email,
        subject: '♠️ Poker Night Results Needed',
        html: `
          <!DOCTYPE html>
          <html>
          <head>
          ...
          <body>
            <div class="container">
              <div class="header">
                <h1>♠️ Poker Championship Tracker</h1>
              </div>
              <div class="content">
                <div class="card-icon">🃏</div>
                <p>Hi ${player.name},</p>
                <p>The poker game from <strong>${gameDate}</strong> still needs final positions to be recorded.</p>
                <p>Please log in to the Poker Championship Tracker to submit the final standings. Every game counts towards the championship!</p>
                <center>
                  <a href="${process.env.APP_URL || 'http://localhost:3000'}" class="button">Submit Results</a>
                </center>
              </div>
              <div class="footer">
                <p>Poker Championship Tracker | Keeping the game fair and competitive</p>
              </div>
            </div>
          </body>
          </html>
...
export const sendWelcomeEmail = async (player) => {
  try {
    const mailOptions = {
      from: process.env.FROM_EMAIL || process.env.SMTP_USER,
      to: player.email,
      subject: '♠️ Welcome to the Poker Championship!',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="container">
            ...
            <div class="footer">
              <p>Poker Championship Tracker | May the odds be ever in your favor</p>
            </div>
          </div>
        </body>
        </html>
